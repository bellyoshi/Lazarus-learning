...
header_match = re.search(r'(unit PdfiumLib;.*?implementation)', content, re.DOTALL)
if header_match:
    header = header_match.group(1)
else:
    # フォールバック：最初の1000行をヘッダーとして使用
    lines = content.split('\n')
    header = '\n'.join(lines[:1000])
...
# 結果を書き込む
with open('Core/PdfiumLib_Minimal.pas', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("最小限のPdfiumLibファイルを作成しました")
